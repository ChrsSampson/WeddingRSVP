    ...
    try{
        if(method === "GET") {
            ...
            try{
                ...
                const userParty = await Party.findById(user.party);
                if(userParty && !userParty.users.includes(id)){
                    userParty.users.push(id);
                    await userParty.save();
                } else if (!data.party && user.party) {
                    userParty.users = userParty.users.filter((user) => user !== id);
                    await userParty.save();
                }
